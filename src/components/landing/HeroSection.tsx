    ...
    <section className="relative pt-32 pb-20 md:pt-40 md:pb-32 overflow-hidden">
      ...
      <div className="container mx-auto px-6">
        <div className="grid lg:grid-cols-2 gap-16 items-center">
          ...
          >
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <Button
                ...
                className="rounded-full px-7 h-12 text-base gap-2"
              >
          ...
          >
            <div className="relative rounded-2xl border border-border bg-card shadow-2xl shadow-primary/5 p-6">
              {/* Fake dashboard header */}
              <div className="flex items-center gap-2 mb-5">
                <div className="w-3 h-3 rounded-full bg-destructive/60" />
                <div className="w-3 h-3 rounded-full bg-warning/60" />
                <div className="w-3 h-3 rounded-full bg-success/60" />
                <span className="ml-3 text-xs text-muted-foreground">Syncareer Dashboard</span>
              </div>
              {/* Mockup cards */}
              <div className="grid grid-cols-2 gap-3 mb-4">
                <DashboardCard icon={<Target className="h-4 w-4 text-primary" />} label="Career Match" value="92%" />
                <DashboardCard icon={<FileText className="h-4 w-4 text-secondary" />} label="CV Score" value="78/100" />
                <DashboardCard icon={<Mic className="h-4 w-4 text-accent" />} label="Interviews" value="3 done" />
                <DashboardCard icon={<Sparkles className="h-4 w-4 text-warning" />} label="Skills" value="12 mapped" />
              </div>
            ...
            <motion.div
              ...
              className="absolute -bottom-6 -left-6 rounded-xl border border-border bg-card shadow-lg p-3 flex items-center gap-3"
            >
              <div className="w-8 h-8 rounded-lg bg-success/10 flex items-center justify-center">
                <Sparkles className="h-4 w-4 text-success" />
              </div>
              <div>
                <p className="text-xs font-medium text-foreground">New match found</p>
                <p className="text-[10px] text-muted-foreground">UX Designer at Andela</p>
              </div>
            </motion.div>
          </motion.div>
        </div>
...
function DashboardCard({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="rounded-xl bg-muted/40 border border-border/50 p-3.5">
      <div className="flex items-center gap-2 mb-1.5">{icon}<span className="text-[11px] text-muted-foreground">{label}</span></div>
