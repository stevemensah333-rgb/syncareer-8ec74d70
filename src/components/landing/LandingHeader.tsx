...
export default function LandingHeader({ onSignIn, onSignUp }: LandingHeaderProps) {
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-[hsl(220,20%,8%)]/80 backdrop-blur-xl border-b border-white/5">
      <div className="container mx-auto px-6 h-16 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          <img src={syncareerLogo} alt="Syncareer" className="h-8 w-auto object-contain" />
          <span className="hidden sm:inline text-xl font-bold text-white tracking-tight">Syncareer</span>
        </div>
        <nav className="hidden md:flex items-center gap-8">
          <a href="#problem" className="text-sm text-white/50 hover:text-white transition-colors">
            Why Syncareer
          </a>
        ...
        <div className="flex items-center gap-2">
          <Button variant="ghost" size="sm" onClick={onSignIn} className="text-white/60 hover:text-white hover:bg-white/10">
            Log in
          </Button>
